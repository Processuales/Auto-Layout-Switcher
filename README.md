...
## Issues
1. There is a *very* minor delay when switching view, because Godot does not have an in-built way to switch layouts through code. The plugin gets around this limitation finding the “Editor Layouts” menu node in the editor UI tree and emitting the same signal that Godot emits when you select a layout manually.
